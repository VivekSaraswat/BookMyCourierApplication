...
//Find a single courier with a pickup Location
exports.findCourier = async (req, res) => {
  try {
    const courier = await CourierModel.findOne({pickupLocation : req.params.pickupLocation});
    res.status(200).json(courier);
  } catch (error) {
    res.status(404).json({ message: error.message });
  }
...
//Find a single courier with customer Id
exports.findCourierByCustomerId = async (req, res) => {
  try {
    const courier = await CourierModel.findOne({customerId : req.params.customerId});
    res.status(200).json(courier);
  } catch (error) {
    res.status(404).json({ messgae: error.message });
  }
